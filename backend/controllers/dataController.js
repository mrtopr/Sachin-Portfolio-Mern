...
const compareAdminName = async (request, reply) => {
  ...
  const db = getDB();
  const admin = await db.collection("SachinPortfolio").findOne({});
  if (!admin) {
...
const compareAdminPassword = async (request, reply) => {
  ...
  try {
    ...
    await db.collection("SachinPortfolioOTP").deleteMany({});
    await db.collection("SachinPortfolioOTP").insertOne({ otp, expireTime });
    reply.send({
      success: true,
      otpSent: true,
      otp: otp,
...
const compareOTP = async (request, reply) => {
  ...
  const db = getDB();
  try {
    const otpData = await db.collection("SachinPortfolioOTP").findOne({});
    ...
    await db.collection("SachinPortfolioOTP").deleteOne({});
  ...
  try {
    ...
    const collections = {
      ...
      yearInReviewTable: await db
        .collection("yearInReviewTable")
        .countDocuments({ deleted: { $ne: true } }),
      SachinPortfolio: await db
        .collection("SachinPortfolio")
        .countDocuments(),
      FeedTable: await db
        .collection("FeedTable")
        .countDocuments({ deleted: { $ne: true } }),
