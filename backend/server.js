...
app.register(require("@fastify/cookie"), {
  cookie: { path: "/", secure: isProduction, httpOnly: true, sameSite: isProduction ? "none" : "lax" },
});
...
// Global endpoints
app.get("/", (req, reply) =>
  reply.send("Welcome to Sachin's MERN Portfolio Backend")
);
app.get("/api", (req, reply) =>
  reply.send("This is the API track for Sachin's MERN Portfolio Backend.")
);
...
(async () => {
  try {
    ...
    // AI context init is non-blocking — server starts even if embeddings fail
    aiContextManager.initContext().then(() => {
      console.log("✅ AI context initialized");
    }).catch((err) => {
      console.warn("⚠️ AI context init failed (will retry on first query):", err.message);
    });
